...
contract Vat {
    ...
    function init(bytes32 ilk) internal { // external auth
        require(ilk_rate == 0, "Vat/ilk-already-init");
        ilk_rate = 10 ** 27;
        emit Init(ilk);
    }

    function file(bytes32 what, uint256 data) internal { // external auth
    ...
    function file(bytes32 ilk, bytes32 what, uint256 data) internal { // external auth
        require(live == 1, "Vat/not-live");
    ...
    // int256 wad => uint256 wad
    function slip(bytes32 ilk, address usr, uint256 wad) internal { // external auth
    ...
    function frob(bytes32 i, address u, address v, address w, uint256 dink, uint256 dart) internal { // external
        // system is live
        require(live == 1, "Vat/not-live");
    ...
    function fold(bytes32 i, address u, uint256 rate_) internal { // external auth
        require(live == 1, "Vat/not-live");
    ...
    function crytic_test_eq_dai() view public returns (bool){
        return (debt == (vice + ilk_Art * ilk_rate));
    }
}